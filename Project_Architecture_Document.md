...
- `[R22]` Session-37/38 markdown-exclusion pass (2026-09-23): the
  pulled main tripped the r21 docs-token pin — the user's
  session_37.md narration quoted the stripped transparent-outline +
  selection-variant tokens, regrowing the production CSS 152,062 →
  152,474 bytes (r22-F1, reworded). The systemic fix (r22-F2):
  every committed `.md` is excluded from TW4 content detection
  (`@source not "../../**/*.md";` — documentation is not a render
  surface) because the regression class recurs every round and the
  pin only caught it at the next round's baseline. The css-hygiene
  stripComments was hardened to a string-aware scanner (the regex
  stripper corrupted the recursive-glob path inside the directive's
  own quotes), a dedicated markdown-exclusion pin added, the
  docs-token scan retained as defense in depth. The session was
  interrupted mid-audit; 334 vitest, all gates green on the
  committed tree.

- `[R23]` Session-39 CSS-drop audit + parity re-verification
  (2026-09-24): completed the interrupted r22 audit with a
  rule-level build diff (the r21 tree rebuilt in this environment at
  152,035 bytes vs the r22 tree's 150,821): the markdown exclusion
  drops NINE dead rules + TEN dead theme-variable emissions — the
  three regrown rules plus six older doc-derived ones (bare
  display-contents, table-row-collapse, coral-background,
  ring-color, and 8px-backdrop utilities — only the -xl backdrop
  variant is ever rendered — and a garbage arbitrary-property rule
  compiled from CLAUDE.md's own error-log example) and ten
  consumerless :root emissions (the inlined body-font variable, two
  legacy canvas-accent colors, six glow-shadow variables whose
  utility rules carry literal fallbacks, and the dead backdrop
  variable). Every dropped item verified dead against src — every
  utility the app renders compiles from a literal class string in
  src (the style maps and component ternaries are TW4-visible);
  nothing real lost, nothing wrong added. Fixes: the r22 commit's
  overclaiming comments corrected to the audited truth; the
  committed session_38.md narration's own tokens reworded (the
  docs-token pin was RED on main — the verify-gate was broken);
  docs/DEPLOYMENT.md created (referenced by .env.example but
  missing since the r16 env rework); full docs alignment (334-test
  counts, the r22-F2 contract, README r22/r23 rows). Full parity
  re-verification: 9/9 battery surfaces at baseline (dashboard
  0.333 / projects 0.305 / supplies 0.305 / inspiration 0.466 /
  scrolled 0.148 / mobile 0.328 / sidebar-drawer 0.577 /
  chat-drawer 1.144 / login 0.232), geometry byte-equal (docH 982 /
  scrollMax 138 / mainH 982; drawers 312px at x=0 and x=78), E2E
  25/25, smoke 23/23, export envelope parity (live account left
  pristine — navigation + export only), dev + production CSS clean.
  334 vitest + 25 E2E, all gates green.

- `[R24]` Session-41 WebKit engine matrix + auth-success scroll
  reset (2026-09-24): the WebKit engine matrix — queued since r20
  because the sandbox lacked GTK — is now measured (the environment
  gained GTK4 + Xvfb; Playwright WebKit 2359 launches through a
  user-space deb-extracted GTK4/Mesa/glib-networking stack under
  Xvfb with llvmpipe software GL). Fresh WebKit baselines across
  the 9-surface battery (login 1.715 / dashboard 2.726 / projects
  0.943 / supplies 0.870 / inspiration 1.162 / scrolled 2.440 /
  sidebar-drawer 0.948 / chat-drawer 3.145 / mobile-dashboard
  7.642%), same-side determinism 0.000% on both sides, desktop
  geometry byte-equal (998/154/998 on both sides — WebKit's own
  text metrics, matching each other exactly), drawers 312px at
  x=0/x=78, and symmetric cross-engine shifts (per-side
  Chromium-vs-WebKit deltas within 0.04% on every surface). The
  mobile dashboard's 7.642% has ONE root cause, and it is on the
  LIVE's side: the Partner-Spotlight label row's "Sponsored" badge
  sits at its wrap threshold (live content 178.28px under WebKit vs
  178.58px under Chromium — the badge wraps to a second line under
  WebKit only; the clone is IDENTICAL to the live under Chromium
  at 178.58px and measures 176.70px under WebKit), cascading a 12px
  shift below that card while every other card matches byte-exactly
  — accepted divergence of the fractional text-row class (the live
  is the engine-unstable side; matching both engines' flip states
  would require engine-targeted CSS breaking reference parity).
  r24-F2 (HIGH): the WebKit mobile flow exposed a REAL behavioral
  divergence — the live's SPA view swap resets the document scroll
  to top on auth success while the clone's router.refresh()
  re-render preserved the pre-submit offset (measured on BOTH
  engines: WebKit pre 201 -> live 0 / clone 201; Chromium
  forced-wheel pre 128 -> live 0 / clone 128; reachable whenever
  the login card exceeds the fold — the submit lands at y=851
  against the 844-fold under WebKit metrics vs 778 under Chromium,
  which is why the Chromium-only battery never saw it). Fixed with
  an explicit scroll-to-top in the login screen's auth-success
  handler, pinned by a login-fidelity unit test (RED -> GREEN) and
  an E2E scroll assertion folded into the existing sign-in
  round-trip spec (no extra sign-in — the 5/min limiter budget).
  Also: the third docs-token occurrence reworded (session_40.md,
  the raw r23 narration — the pin was RED on pulled main, the
  verify-gate broken, third occurrence of the documented
  recurrence class); the slow-network probe closed at parity
  (CDP Slow-3G: first-paint 0.149 / settled 0.232 / dashboard
  0.548%); the r22-M1 gradient-compositor race study closed (16
  fresh-navigation captures, zero flat-window artifacts, 0.000%
  intra-side drift on both sides). Chromium re-verified at the
  documented baselines post-fix (9/9, mobile dashboard 0.328%).
  335 vitest + 25 E2E, all gates green.

- `[R25]` Session-42/43 sign-up pre-submission state machine + the
  error/reset-view engine matrix (2026-09-24): the queued error/reset
  engine probe (the battery covered steady-state only) exposed a real
  interactive parity gap — the deployed Amplify signUp and
  reset-confirmation forms validate PER FIELD ON BLUR (measured
  keystroke-level, 20+ states): the pw field's first blur renders the
  Cognito policy stack (empty pw = all five lines) and live-updates on
  every keystroke; the confirm field's first blur renders the mismatch
  line (including an emptied confirm); pristine forms render nothing;
  a submit validates all fields at once; the sign-in form has no
  pre-submission validation. The sign-up submit disables exactly while
  a validation line renders (bg #EFF0F0 / text #89949F / not-allowed;
  email format irrelevant — bad email + valid matching pw stays
  enabled while form.checkValidity() is false); the sign-in and both
  reset submits never disable. Plus the cursor contract (the live's
  eight amplify-button chrome elements compute pointer; the clone
  computed default). Fixed TDD-first (13 pins + a 3-spec zero-auth E2E
  suite): the blur-gated engagement model with derived rendering, the
  disabled chrome, and the cascade-layers-safe cursor rule
  (.ast-amplify-button:not(:disabled) — unlayered rules outrank TW4's
  @layer utilities; without the scope the disabled:cursor-not-allowed
  utility loses, the trap the E2E spec caught). State-for-state
  verification: disabled/cursor/bg/color/stack-geometry byte-equal in
  every measured state. The error/reset views pixel-baselined on three
  engines (Chromium 0.125-0.352%, WebKit 1.563-2.190% fresh baselines
  with 0.000% determinism and cross-engine asymmetry <= 0.017%, Firefox
  0.287-0.421% + the r24 scroll fix verified engine-independent: live
  201 -> 0 / clone 201 -> 0). The SKILL.md distillation was stale at
  r16 and refreshed (frontmatter, Appendix A ledger through r24,
  baselines, surface count). 348 vitest + 28 E2E, all gates green.

- `[R26]` Session-44/45 modal button-row + detail-heading chrome
  (2026-09-26): a fresh live-vs-clone survey with matched test data on
  both sides found TWO visual divergences and fixed both TDD-first
  (4 RED pins -> GREEN, 10 pins total incl. the new
  `src/lib/project-fidelity.test.ts`). r26-F1: both create modals'
  Cancel buttons carried text-sm (14px/20px) while the live's compute
  16px/24px — the bordered Cancel is the flex-stretch row's tallest
  child at 42px and sizes BOTH row buttons, so the clone rendered a
  40px row and a modal card 2px short (542 vs 544 desktop, 792 vs 794
  supply); fixed by dropping text-sm. r26-F2: both detail panels' H2s
  carried a flex flex-wrap items-center gap-2 row while the live's H2
  is a plain text-lg font-bold block — the flex row doubled the
  title-to-NEW-badge gap to 16px (gap-2 + the badge's own ml-2),
  inflated the badge to 20px (vs the live's inline 18px), and shifted
  it 2px up; fixed by removing the flex tokens. Post-fix metrics
  byte-exact (modal 544 @ y150 / 794; buttons 42px at 16px/24px; H2
  193px wide; badge 18px at +6.25). The survey also re-confirmed the
  mobile scroll architecture at parity (page 1362 / section 1220 /
  scrollHeight 1218 with the edit panel open — both sides) and
  documented the twin-copy state-instance split (the r18 accepted
  divergence's consequence) as an AGENTS.md capture-methodology note.
  358 vitest + 28 E2E, all gates green.
- `[R27]` Session-47 login-chrome pass (2026-09-26): the login-card
  battery the steady-state rounds never measured at depth. FIVE findings,
  all in the login chrome: (F1, HIGH) the login inputs render 16px/24px
  on 8px/16px padding on the live (amplify-input metrics, measured on
  every view at both viewports) while the clone carried text-sm/px-3
  (14px/20px on 12px — the typed text and placeholders 2px smaller,
  capture-visible); (F2, MEDIUM) the eye toggle's a11y contract — the
  live keeps the accessible name CONSTANT ("Show password") and
  announces the flip through an sr-only aria-live="polite" span
  ("Password is hidden"/"Password is shown"), the clone relabeled to
  "Hide password" and never announced; (F3, MEDIUM) the reset EMAIL
  view's card is content-sized below md (307px at 390 — the h3 at 32px
  drives the max-content) while the confirmation view stays full-width;
  (F4, MEDIUM) both reset forms pad p-8 uniform (the r9 pb-5 pin was a
  bad measure — 12px card shortfall); (F6) the below-md auth card is
  content-driven at 357px (the r10-accepted 0.5px fractional centering —
  now REPRODUCED with the measured max-w-[357px] md:max-w-[480px] pair,
  byte-exact 357.00 @ x16.50 on both sides, mobile login capture 0.93%
  → 0.14%). All fixed TDD-first (6 RED pins → GREEN, 362 vitest). Also
  probed CLEAN: the create modals at 390×844 (session_45's next-step),
  the duplicate-email alert (byte-exact, doc-y 868/942), the steady-state
  capture battery with pre-capture state verification (0.22–0.97%
  mobile / 0.35–0.54% desktop), and the mobile chat drawer. The
  paired-capture rule hardened: assert the scrim's DOM presence, the
  drawers' translate-x, scrollY, and the visible h1 on BOTH sides before
  every capture (offsetParent is null for fixed elements — a visibility
  probe built on it silently passes while a drawer is open; the first
  projects capture false-diffed 13.38% on an open live chat drawer).
  362 vitest + 28 E2E + 23/23 smoke, all gates green.
...
| E2E | 28 specs | `e2e/*.spec.ts` — setup (storageState sign-in), auth, signup-validation (the r25 blur-gated pre-submission machine, zero auth requests), dashboard, import-export, projects, supplies, mobile-navigation | Playwright (serial, 1 worker; desktop 1536×844 + mobile 390×844 projects) |
...
| Medium | ~~The documentation-token regression class recurred every round~~ | Raw session narrations naturally quote prior-session stripped tokens; the r21 pin caught each occurrence only at the NEXT round's baseline, leaving the CSS regressed on main in between (measured: session_37.md regrew the production CSS 152,062 → 152,474 bytes) | **Resolved 2026-09-23 (r22)** — every committed `.md` file excluded from TW4 content detection (`@source not "../../**/*.md";` — documentation is not a render surface); the string-aware stripComments scanner, a dedicated markdown-exclusion pin, and the docs-token scan retained as defense in depth |
| Medium | ~~The r22 markdown-exclusion build diff was unaudited~~ | The interrupted session-38 left the exclusion's CSS drop unaccounted (post-fix CSS 1,241 bytes below the r21 record) with the r22 comments claiming "drops exactly those dead rules and nothing else" — an unverified claim in committed comments | **Resolved 2026-09-24 (r23)** — rule-level build diff completed: nine dead rules + ten dead theme-variable emissions dropped (152,035 → 150,821 bytes), every one verified consumerless against src (no rendered surface uses them; every utility the app renders compiles from a literal class string in src); the overclaiming comments corrected; production CSS verified at 150,821 bytes with 0 forced-colors / 0 selection / 0 doc-derived rules |
| High | ~~The docs-token pin was RED on main~~ | The committed r22 narration (session_38.md) itself quoted the transparent-outline token — the exact recurring class, tripping the pin on the pulled tree and breaking the verify-gate on main | **Resolved 2026-09-24 (r23)** — the two token-bearing lines reworded to descriptive references; 334/334 vitest green; the pin stays as the canary for the pin-vs-exclusion contract |
| Medium | ~~Auth-success scroll behavior diverged~~ | The live's SPA view swap resets the document scroll to top on successful sign-in/sign-up; the clone's router.refresh() re-render preserved the pre-submit scroll offset — reachable whenever the mobile login card exceeds the viewport fold (every real phone; invisible to the Chromium battery whose tighter text metrics keep the submit at y=778 inside the 844 fold) | **Resolved 2026-09-24 (r24)** — explicit window.scrollTo(0, 0) in the login screen's auth-success handler (measured both engines: WebKit pre 201 → live 0 / clone 201; Chromium forced-wheel pre 128 → live 0 / clone 128); pinned by a login-fidelity unit test and an E2E scroll assertion in the sign-in round-trip spec |
| High | ~~The docs-token pin was RED on main (third occurrence)~~ | The committed r23 narration (session_40.md) quoted the transparent-outline token verbatim at line 13 — the same recurring class (sessions 37 → 38 → 40), breaking the verify-gate on pulled main again | **Resolved 2026-09-24 (r24)** — the line reworded to a descriptive reference; 335/335 vitest green; the recurrence class remains structural (raw narrations quote tokens) and the markdown exclusion keeps the compiled CSS immune — the pin is the canary, not the defense |
| Low | ~~.env.example referenced a missing deployment doc~~ | The r16 env rework added a "see docs/DEPLOYMENT.md §4" pointer, but the file was never created | **Resolved 2026-09-24 (r23)** — docs/DEPLOYMENT.md created (build, database setup, environment variables incl. the absolute-path production guidance, running, and the pre-exposure security checklist) |
...
| Low | ~~Mobile login card offset by 0.5px~~ | The live's card is 357px centered in its 358px content column (x=16.5 — Amplify internal responsive fractional layout); the clone filled the column (358px at x=16). Desktop was byte-identical (480×429 at 494,464) | **Resolved 2026-09-26 (r27)** — the measured cap `max-w-[357px] md:max-w-[480px]` reproduces the live's content-driven width at every viewport (375 fills 343 like the live; 390 centers 357.00 @ x16.50 byte-exact; md+ 480.00 @ x174.00); pinned by a login-fidelity test with the full measurement record |
...
| Low | Twin copies hold separate view-state instances (r26) | Measured 2026-09-26: the live's hidden desktop copy and visible mobile copy each hold their OWN React view-state instance — interactions on one copy leave the other's sub-view state (supplies grid/list/detail/edit) at its own last value, so crossing the md breakpoint mid-session surfaces that copy's state; the clone's single responsive copy keeps one state across the crossing. Invisible in every steady-state capture (both copies render the same shared app-level view; only the sub-view differs) | Accepted (consequence of the r18 single-copy divergence — replicating it would require the twin-copy structure the landmark pin rejects; documented as an AGENTS.md capture-methodology note) |
...
| Low | ~~WebKit engine unmeasured~~ | The engine matrix covered Chromium + Firefox only; Playwright's WebKit could not launch in the r20 sandbox (missing GTK-4/libsoup system libraries — an environment limitation, not a repo defect) | **Resolved 2026-09-24 (r24)** — the sandbox gained GTK4 + Xvfb and the matrix ran via a user-space GTK4/Mesa/TLS stack: 9-surface paired battery at fresh WebKit baselines, 0.000% same-side determinism on both sides, byte-equal desktop geometry, symmetric cross-engine shifts (≤ 0.04%/surface), and ONE accepted divergence documented (the live's own Partner-Spotlight badge wrap is engine-unstable at its threshold — the clone is Chromium-identical) |
| Low | ~~The sign-up/reset pre-submission validation machine diverged~~ | The live's Amplify forms render client validation PRE-SUBMISSION (pw blur -> policy stack, live-updating; confirm blur -> mismatch line) and the sign-up submit flips to a disabled gray chrome exactly while a line renders; the clone rendered nothing until submit and its submit never disabled (r9 measured the submitted states, r20 the sign-in round-trip — the pre-submission machine was never probed until the r25 error-view engine pass) | **Resolved 2026-09-24 (r25)** — the blur-gated per-field engagement model with derived stack/mismatch rendering, the measured disabled chrome, and the cursor contract; state-for-state verified on the live (13 pins + a zero-auth E2E suite) |
| Low | ~~The amplify-button cursor contract diverged~~ | The live's eight login chrome elements computed cursor: pointer; the clone's computed default (invisible in captures — a computed-style divergence of the class the repo pins since r20) | **Resolved 2026-09-24 (r25)** — one cascade-layers-safe globals rule (.ast-amplify-button:not(:disabled) { cursor: pointer }); the scope is load-bearing (an unlayered rule outranks TW4's @layer utilities) |
