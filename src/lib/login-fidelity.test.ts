 ...
 *   sibling, the 16px/700 submit, and the 51px #047d95 "Forgot your
 *   password?" row.
 ...
 *   instead of an inline notice. "Send code" surfaces the support notice
 *   (no mailer exists — ADR-003).
 * - Sign-in error: the live's exact Amplify copy "Incorrect username or
 *   password." rendered in #660000 at 16px (measured on the live DOM).
...
describe("login card chrome (live Amplify geometry)", () => {
  ...
  it("renders the forgot row at the live's 51px teal block", () => {
    expect(login).toMatch(/h-\[51px\][^>]*text-\[#047d95\]/);
  });
});
...
describe("reset password view (live Amplify forgot flow)", () => {
  ...
  it("surfaces the support notice as the Send code result (no mailer)", () => {
    expect(login).toContain("support@artsupplytracker.com");
  });
});
...
describe("sign-in error copy (live Amplify)", () => {
  ...
  it("renders the error in the live's #660000 at 16px", () => {
    expect(login).toMatch(/role="alert"[^>]*text-base text-\[#660000\]/);
  });
});
