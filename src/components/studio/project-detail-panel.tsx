 ...
 * image (photo or the live app's decorative placeholder), Budget, the
 * Supplies assignment section ("Pick supply…" select + Assign, assigned
 * list with × remove), Delete (native confirm, exact live copy), and
 * Edit Project.
 *
...
import { projectStatusLabel } from "@/lib/studio-domain";
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-ast-body">
      ...
      <div className="mb-4 border-t border-ast-turquoise/20 pt-4">
        ...
        {unassigned.length > 0 ? (
          ...
          <p className="text-xs text-ast-body/50">
            {supplies.length === 0
              ? "Add supplies to your studio to assign them here."
              : "All supplies assigned"}
          </p>
        )}
      </div>
 ...
 * otherwise a dark gradient with faint rotated accent lines.
 */
function ImagePlaceholder({ photo, name }: { photo: string | null; name: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-2 left-5 h-px w-8 rotate-3 bg-ast-lavender/20" />
    </div>
  );
}
