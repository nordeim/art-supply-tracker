...
- **TW4 scans MARKDOWN too — documentation can regress the CSS (r21)** —
  the r19/r20 selection/forced-colors remediations were undone by their
  own session records: the stripped tokens were quoted verbatim in the
  ...
  pair). Never write a stripped token (or any unused class-shaped
  string) literally in ANY committed file — refer to it descriptively
  ("the transparent-outline utility") and pin with runtime-constructed
  matchers; `css-hygiene.test.ts` now scans every .md in the repo for
  the two regressed token families.
