...
| 📱 Responsive | Desktop: three glass cards (sidebar / main / chat) in a 12-column grid, each rounded-3xl on the blurred `#0B0018` canvas with per-panel accent borders — the shell is uncapped like the live's (the grid row sizes to the chat column's content; the page grows past the fold and the window scrolls); mobile: slide-in drawers with the "☰ Studio Tools" / "Chat ☰" toggle bar |
...
`bun run test:e2e` (Playwright, 28 specs) drives the golden paths
...
Automated tests (Vitest, 362 tests) pin the studio-domain vocabulary
...
md-toggled content copies), the CSS compile hygiene (r19: the
`@source not "../../skills"` directive keeping the skills corpus out of
Tailwind's automatic content detection — the repo's task contract
excludes skills/ from compilation, and without the directive TW4 compiles
class-like strings from skill docs into the app's CSS as dead rules, a
measured 37% stylesheet inflation — plus the nothing-authored
selection/caret contract: no `::selection` rules and no `caret-color`
anywhere, matching the live's CSSOM, with the shadcn Input's scaffold
selection utilities stripped), the forced-colors compile hygiene (r20:
no source token for TW4's forced-colors-aware transparent-outline
utility — the live authors zero `@media (forced-colors: active)`
rules), the login-motion fidelity (r20: the login chrome's measured
Amplify transition contract — `all 0.25s ease` on every login element
with the property-none override on the ACTIVE tab, the scoped
`.ast-amplify-button` reduced-motion guard replicating the live's
`.amplify-button` rule, and the constant pending-state contract — the
live's submit never swaps its label or dims), and
...
| CSS compile-hygiene pass (r19) | ✅ Complete | A fresh full-surface recon on the r18 tree (8 paired captures at or below the documented baselines — dashboard 0.34%, projects 0.34%, supplies 0.34%, inspiration 0.49%, mobile-dashboard 0.17%, sidebar drawer 0.47%, chat drawer 0.76%, login 0.17%; export envelope byte-identical modulo timestamp; 23/23 smoke; drawer geometry byte-identical 312×844 z-50, chat x=78) plus the three session-30-suggested probe dimensions. Caret/selection/placeholder rendering: PARITY — probed input-by-input on both sides (the chat input, the file inputs, and every Create Project modal field): caret `rgb(255,255,255)`, `::selection` transparent (UA default), placeholder white at 40%, the photo input's lavender `rgb(220,199,255)` chrome identical; the clone's oklab-serialized alpha composites verified numerically equal to the live's rgba values (the documented sub-perceptual compositing class). `prefers-contrast: more` emulated rendering: PARITY — zero authored contrast/forced-colors rules in either CSSOM under active emulation, paired captures at the standard baselines (desktop 0.34%, mobile 0.17%). devicePixelRatio matrix: PARITY — dpr1 at 0.34% (the standard baseline), dpr2 (3072×1688) at 0.40% (2x text rasterization, within the accepted envelope); no DPI-dependent layout divergence. TWO real findings fixed: r19-F1 (HIGH, compile hygiene): Tailwind v4's automatic content detection scanned the committed `skills/` folder — violating the repo's skills-excluded-from-compilation contract — leaking class-like strings from skill docs into the compiled CSS as dead rules (the gift-evaluator skill's HTML template contributed two red-tinted `::selection` rules) and inflating the stylesheet 37% (300,314 → 189,577 bytes dev; 3147 → 1922 CSSOM rules vs the live's 1442); fixed with `@source not "../../skills";` in globals.css, rendering verified 100.00% identical. r19-F2 (MEDIUM): the shadcn Input scaffold's selection-variant bg-primary / text-primary-foreground utility pair — dead while unused but divergent the moment consumed (the live authors zero `::selection` rules) — stripped from `src/components/ui/input.tsx`; the compiled CSS now carries zero `::selection` rules, matching the live's CSSOM. Also r19-M1 (methodology): the live's Vite SPA applies styles asynchronously in fresh headless contexts — the first Playwright paired run measured 5.19% until decomposed (live-vs-live across browser contexts 4.97%, clone-vs-clone 0.00% — the clone renders deterministically across engines); fresh-context captures now gate on a paint-settle condition. Pinned by the new `css-hygiene.test.ts` (5 pins, TDD red→green verified via git-stash of the fixes). 315 tests, 25 E2E, all gates green, production build + production CSS verified (0 skills-sourced utilities) |
| Media-contract pass (r20) | ✅ Complete | A fresh full-surface recon on the r19 tree (8 paired captures at or below the documented baselines — dashboard 0.38%, projects 0.37%, supplies 0.37%, inspiration 0.53%, mobile-dashboard 0.17%, sidebar drawer 0.47%, chat drawer 0.76%, login 0.17%; export envelope byte-identical modulo timestamp; 23/23 smoke; drawer geometry byte-identical 312×844 z-50, chat x=78) plus the three session-32-suggested probe dimensions. `forced-colors: active` emulated rendering: PARITY in captures (desktop 0.35% / mobile 0.14%) — but the probe's CSSOM media-rule sweep surfaced a real divergence: the clone's compiled CSS carried SIX `@media (forced-colors: active)` rules (TW4's forced-colors-aware transparent-outline utility, emitted from the unused shadcn scaffold's class strings) while the live authors zero — fixed by stripping the 29 scaffold tokens (six dead rules gone; dev CSS 189,849 → 188,388 bytes; production CSS 153,077 → 152,241 with 0 forced-colors rules; rendering unchanged, all 8 screenshots byte-identical). `prefers-reduced-transparency: reduce` (CDP-engaged, verified): PARITY — 0.34%/0.17%, zero authored rules in either CSSOM. Firefox engine matrix (WebKit cannot launch in the sandbox — missing GTK system libraries): PARITY — 8/8 paired surfaces at/below the Chromium baselines (0.36/0.32/0.32/0.50/0.17/0.30/0.31/0.18%), 0.00% cross-context determinism on BOTH sides, and the ~7.7% cross-engine delta is symmetric (live 7.83% / clone 7.71% — engine font rasterization shifts both sides equally). Decomposing the CSSOM media-class inventory also surfaced the login chrome's transition contract (r20-F2): the live's login elements compute `transition: all 0.25s ease` (inactive tabs, inputs, eye, submits, links, Dismiss) with `property: none` on the ACTIVE tab, and `.amplify-button { transition: none }` under `prefers-reduced-motion: reduce` (the r17 "no reduced-motion guard" record was incomplete — its inventory counted @keyframes only; the live carries five reduced-motion MEDIA rules, four dead on both sides and documented); the clone had the TW utility's 0.15s fades and no guard — now `transition-all duration-[250ms] ease-[ease]` everywhere measured (computed byte-identical to the live, verified per element), the scoped `.ast-amplify-button` guard, and the ACTIVE tab override. Plus r20-F3: the live's login submit holds label/enabled/opacity CONSTANT through the auth round-trip (measured at 120ms intervals) — the clone's "Signing in…" swap and disabled dim removed. Pinned by 13 new tests (css-hygiene extension + the new login-motion-fidelity suite + the refined r17 motion pin). 328 tests, 25 E2E, all gates green, production build + CSS verified |
| App-shell height pass (r21) | ✅ Complete | A fresh full-surface recon on the r20 tree (8 paired captures with a re-derived hot-pixel metric — sum-of-channel-deltas > 30; export envelope byte-identical; 23/23 smoke; drawer geometry byte-identical; mobile surfaces unchanged) surfaced ONE structural finding: **r21-F1 (HIGH)** — the live's desktop app shell is UNCAPPED (its grid row sizes to the chat column's intrinsic content — sticky header 240 + the scroll container's max-h calc 588 + padding = 878px — so the page grows to 982 at 1536×844 and the WINDOW SCROLLS, wheel-verified 0→138; the sidebar/chat cards stretch past the fold), while the clone carried the scaffold's viewport-height cap from the initial commit (never live-measured): the page was locked at 844, the chat card clipped at 740px with its bottom border + rounded corners rendered where the live's card continues, and the wheel scroll was dead (~0.43% hot strips on every desktop surface). Fixed by removing the cap token — the clone's geometry now matches the live's numbers EXACTLY (main 982 / grid row 878 / chat aside 878 / wheel 138, verified on dev AND production servers); the desktop paired captures dropped to 0.33/0.31/0.31/0.47% — below every baseline, the remaining regions being the accepted account-email strip. Plus **r21-F2 (MEDIUM)**: the r19/r20 selection/forced-colors remediations had been regressed by their own documentation — the stripped tokens were quoted verbatim in the committed .md files and TW4's content detection (which scans markdown too) compiled the dead rules back into the CSS (the "clean" 152,241-byte stylesheet had regrown to 152,390); all 13 literal occurrences reworded descriptively and pinned by a new docs-token scan (production CSS now 152,062 bytes — 0 forced-colors, 0 selection rules, cleaner than the r20 record). Mobile verified byte-stable (no mobile change; the reference re-shoot differs from the r20 references only by environment-level sub-pixel text rasterization — VLM-verified identical content at 2× zoom). 333 tests (5 new), 25 E2E, 23/23 smoke, all gates green, production build + CSS verified |
| Markdown-exclusion pass (r22) | ✅ Complete | The r21 docs-token pin fired on the freshly pulled main: the user's `session_37.md` commit (the raw r21 narration) quoted the stripped transparent-outline + selection-variant tokens verbatim, and TW4's markdown scanning regrew the production CSS from 152,062 to 152,474 bytes (one forced-colors block + the selection pair) — the exact r21-F2 mechanism, caught by the pin exactly as designed. **r22-F1**: the two token-bearing lines reworded to descriptive references. **r22-F2 (the systemic fix)**: because the class recurs EVERY round (raw narrations naturally quote prior-session tokens, and the pin only catches it at the NEXT round's baseline — leaving the CSS regressed on main in between), every committed `.md` file is now excluded from TW4 content detection entirely (`@source not "../../**/*.md";` in globals.css — documentation is not a render surface). The css-hygiene pin's `stripComments` was also hardened (string-aware scanning — the old regex-based stripper corrupted the recursive-glob shape inside the directive's own quoted path), a dedicated markdown-exclusion pin added, and the docs-token scan retained as defense in depth. The round was interrupted mid-audit (the build-diff accounting of everything the exclusion dropped); r23 completed it — see the next row. 334 tests, all gates green |
| WebKit engine matrix + scroll-reset pass (r24) | ✅ Complete | The WebKit engine matrix — queued since r20 because the sandbox lacked GTK — is now MEASURED: the sandbox gained GTK4 + Xvfb, and Playwright WebKit 2359 launches via a user-space deb-extracted GTK4/Mesa/TLS stack under Xvfb with software GL (llvmpipe). Full 9-surface paired battery at the live's canonical viewports: fresh WebKit baselines login 1.715 / dashboard 2.726 / projects 0.943 / supplies 0.870 / inspiration 1.162 / scrolled 2.440 / sidebar-drawer 0.948 / chat-drawer 3.145%, same-side determinism 0.000% on BOTH sides, desktop geometry byte-equal (docH 998 / scrollMax 154 / mainH 998 on both sides — WebKit's own text metrics, matching each other exactly), chat drawer 312px @ x=78 on both sides, and the cross-engine check is textbook symmetric (Chromium-vs-WebKit per side differs by ≤ 0.04% on every surface — engine rasterization shifts both sides equally, the r20 Firefox pattern at larger magnitude under software rasterization). The mobile dashboard's elevated 7.642% decomposes to ONE root cause: the live's OWN Partner-Spotlight label row is engine-unstable at its wrap threshold (its content measures 178.28px under WebKit vs 178.58px under Chromium — the "Sponsored" badge flips to a second line on the live only; the clone measures 176.70px under WebKit and 178.58px — IDENTICAL to the live — under Chromium), cascading a 12px shift below that card; every OTHER card matches byte-exactly (82/472/208/214px) — accepted divergence of the fractional text-row class, with the live itself the unstable side. **r24-F2 (HIGH, the round's parity fix)**: the battery's WebKit mobile flow exposed a real behavioral divergence — the live's SPA view swap resets the document scroll to top on auth success, while the clone's router.refresh() re-render PRESERVED the pre-submit offset (measured both engines: WebKit mobile pre 201 → live 0 / clone 201; Chromium forced-wheel pre 128 → live 0 / clone 128 — reachable whenever the login card exceeds the fold, which is every real phone: the submit lands at y=851 against the 844 fold under WebKit metrics). Fixed with an explicit scroll-to-top on auth success, pinned by a login-fidelity unit test (RED → GREEN) and an E2E scroll assertion folded into the existing sign-in round-trip spec (no extra sign-in — the rate-limiter budget is a pinned contract). Also fixed: the third docs-token occurrence (session_40.md — the raw r23 narration; the pin was RED on pulled main again, verify-gate broken — reworded to a descriptive reference), and the two remaining queued probes closed: slow-network first-paint at parity (CDP Slow-3G: login first-paint 0.149% / login settled 0.232% / dashboard 0.548%) and the r22-M1 gradient-compositor race study found NO flat-window artifact in 16 fresh-navigation captures with 0.000% intra-side drift on both sides. Chromium re-verified at the documented baselines post-fix (9/9, mobile dashboard 0.328%). 335 tests, all gates green |
| Sign-up pre-submission state machine + error-view engine matrix (r25) | ✅ Complete | The error/reset-view engine probe (the session_41 queued dimension — the battery covered steady-state only) exposed a REAL interactive parity gap the prior rounds' measurements had missed (r9 measured the SUBMITTED states; r20 measured the sign-IN round-trip; neither covered the pre-submission machine). Measured keystroke-level on the live (20+ states): the deployed Amplify signUp/reset forms validate PER FIELD ON BLUR — the pw field's first blur renders the Cognito policy stack (empty pw = all five lines) and live-updates it on every later keystroke; the confirm field's first blur renders the mismatch line (including an emptied confirm); pristine forms render nothing regardless of content; a submit (click or Enter) validates every field at once; the sign-IN form has no pre-submission validation at all. The sign-up submit flips to a measured disabled chrome exactly while a validation line renders (bg #EFF0F0 / text #89949F / cursor not-allowed; email format irrelevant — bad email + valid matching pw stays enabled while form.checkValidity() is false), while the sign-in and both reset-view submits never disable. Plus the cursor contract: the live's eight amplify-button chrome elements compute cursor: pointer, the clone's computed default. Fixed (TDD, 13 new pins + a 3-spec zero-auth E2E suite): the blur-gated pwTouched/confirmTouched engagement model with derived stack/mismatch rendering (replacing the submit-only state), the disabled chrome with the cascade-layers-safe cursor rule (.ast-amplify-button:not(:disabled) — unlayered rules outrank TW4's @layer utilities, the trap the E2E spec caught), and state-for-state verification (live vs clone: disabled/cursor/bg/color/stack-geometry byte-equal in every measured state). The error/reset views are now pixel-baselined on THREE engines: Chromium 5/5 at 0.125–0.352%, WebKit 5/5 at fresh baselines 1.563–2.190% with 0.000% determinism and cross-engine asymmetry ≤ 0.017% (the r24 symmetric engine-noise pattern), Firefox third-engine opinion on the r24 scroll fix (live 201→0 / clone 201→0 — engine-independent) with mobile surfaces at 0.287–0.421% and byte-equal desktop geometry. Chromium battery re-verified at the documented baselines post-fix (9/9); all 9 reference screenshots re-shot at 0.0000% vs the r24 references (visually inert). 348 tests, 28 E2E, 23/23 smoke, all gates green |
| Modal button-row + detail-heading chrome pass (r26) | ✅ Complete | A fresh live-vs-clone survey with test data on both sides (the pristine account drove the create/detail/edit surfaces through real rows on each — deleted through the UI afterward): dashboard, projects, supplies, inspiration, both create modals, both detail panels, both edit panels, chat send, mobile drawers, mobile layout architecture, chip default/selected states, status tiles, and modal field metrics. TWO real findings, both fixed TDD-first (4 RED pins → GREEN; +10 pins total incl. the new `project-fidelity.test.ts`). r26-F1 (HIGH): both create modals' Cancel buttons carried `text-sm` (14px/20px) while the live's compute 16px/24px — the bordered Cancel is the flex-stretch row's tallest child at 42px and sizes BOTH row buttons, so the clone rendered a 40px row, a modal card 2px short (542 vs 544 desktop, 792 vs 794 supply), and visibly smaller Cancel text; fixed by dropping text-sm from both Cancel class strings. r26-F2 (MEDIUM): both detail panels' H2s carried `flex flex-wrap items-center gap-2` while the live's H2 is a plain `text-lg font-bold text-ast_cyan` block — the flex row doubled the title→NEW-badge gap to 16px (gap-2 + the badge's own ml-2), inflated the badge to 20px (vs the live's inline 18px), and shifted it 2px up; fixed by removing the flex tokens from both H2s (the badge keeps its own ml-2 + align-middle inline flow). Post-fix verification byte-exact on every measured metric: modal card 544 @ y150 / 794, Cancel + Create/Add 42px at 16px/24px, H2 width 193 (live 192.69), badge 18px at +6.25 offset (live 6.25), supply-detail pixel diff back to the accepted header/chat-data-only set; the rest of the survey confirmed at parity (edit-panel buttons 42/16, detail action buttons 38/14, chips, tiles, drawers, mobile scroll architecture 1362/1220/1218 both sides). Also documented the twin-copy state-instance split (the r18 accepted divergence's consequence: the live's desktop and mobile content copies hold separate view-state instances; the clone's single copy keeps one state across viewport crossings) as an AGENTS.md capture-methodology note. All 9 reference screenshots re-shot; 358 tests, 28 E2E, 23/23 smoke, all gates green |
| Login-chrome pass (r27) | ✅ Complete | The login-card battery the steady-state rounds never measured at depth (session_45's next-step list: the create modals at the MOBILE viewport — clean, byte-exact field geometry on both modal cards; the duplicate-email Create Account alert — clean, byte-exact at doc-y 868 with the 50×34 Dismiss; the paired capture battery with pre-capture state verification — clean, 0.22–0.97% mobile / 0.35–0.54% desktop, every surface in the accepted set). FIVE findings, all in the login chrome, fixed TDD-first (6 RED pins → GREEN, 362 vitest): the login inputs' font metrics (the live's amplify-input computes 16px/24px on 8px/16px padding — the clone's text-sm/px-3 rendered the typed text and placeholders 2px smaller, capture-visible; now px-4 py-2 text-base with the deterministic h-[42px]); the eye toggle's a11y contract (the live keeps the accessible name CONSTANT "Show password" and announces the flip through an sr-only aria-live span reading "Password is hidden"/"Password is shown" — the clone's "Hide password" relabel removed); the reset EMAIL view's content-sized card (307px at 390, the h3 at 32px driving the max-content — w-fit md:w-full on the reset ternary; the confirmation view stays full-width); both reset forms' uniform p-8 (the r9 pb-5 pin was a bad measure — 12px card shortfall); and the below-md auth card's content-driven 357px cap (the r10-accepted 0.5px fractional centering now REPRODUCED — max-w-[357px] md:max-w-[480px], byte-exact 357.00 @ x16.50 on both sides, mobile login capture 0.93% → 0.14%, desktop 0.23% → 0.04%). The paired-capture rule hardened: assert state equality (scrim DOM presence, drawer translate-x, scrollY, visible h1) on BOTH sides before every capture — offsetParent is null for fixed elements, and a first-pass projects capture false-diffed 13.38% on an open live chat drawer. All 9 reference screenshots re-shot; 362 tests, 28 E2E, 23/23 smoke, all gates green |

| CSS-drop audit + parity re-verification (r23) | ✅ Complete | Completed the interrupted r22 audit with a rule-level build diff (r21 tree vs r22 tree): the markdown exclusion drops NINE dead rules + TEN dead theme-variable emissions (152,035 → 150,821 bytes) — the three session_37 regrown rules plus six older rules that had silently compiled from the repo's own token tables and prose for many rounds (bare display-contents / table-row-collapse / coral-background / ring-color / 8px-backdrop utilities — only the -xl backdrop variant is ever rendered — and a garbage arbitrary-property rule compiled from CLAUDE.md's own error-log example), plus ten consumerless `:root` variable emissions (the inlined body-font variable, two legacy canvas-accent colors kept in the token table for reference, six glow-shadow variables whose utility rules carry literal fallbacks, and the dead backdrop variable). EVERY dropped item verified dead against src (no rendered surface uses any of them; every utility the app renders compiles from a literal class string in src — the style maps and component ternaries are all TW4-visible); nothing real lost, nothing wrong added. The r22 commit's overclaiming comments ("drops exactly those dead rules and nothing else") corrected to the audited truth. Also fixed: the r22 narration's own committed tokens (session_38.md — the docs-token pin was RED on main, verify-gate broken), the `.env.example` reference to the then-missing `docs/DEPLOYMENT.md` (now created), and the full docs alignment. Full parity re-verification: 9/9 battery surfaces at baseline (dashboard 0.333 / projects 0.305 / supplies 0.305 / inspiration 0.466 / scrolled 0.148 / mobile 0.328 / sidebar-drawer 0.577 / chat-drawer 1.144 / login 0.232), geometry byte-equal (docH 982 / scrollMax 138 / mainH 982; drawers 312px @ x=0 and x=78 on both sides), E2E 25/25, smoke 23/23, export envelope parity (live account left pristine), dev + production CSS clean (0 forced-colors, 0 selection, 150,821 bytes). 334 tests, all gates green |
...
| Verification | ✅ Complete | Lint + typecheck + 334 vitest + 25 Playwright E2E + production build clean; VLM screenshot comparison of all four views (plus the mobile views, the sidebar drawer, and the login page at both viewports — including the error states and the reset-confirmation view) vs the live app (parity); 23-check functional smoke suite (CRUD, filters, panels, modals, navigation regression, focus flows); zoom/reflow (200% + 320px), forced-colors inventory, caret/selection/placeholder rendering, `prefers-contrast: more` emulated captures, the devicePixelRatio 1x/2x matrix, `forced-colors: active` emulated captures, `prefers-reduced-transparency: reduce` (CDP-engaged), the Firefox engine matrix (with cross-context determinism controls), the WebKit engine matrix (GTK4/Xvfb/software-GL launch, 9-surface paired battery with fresh baselines, same-side determinism, and symmetric cross-engine shifts), slow-network first-paint captures (CDP Slow-3G), the gradient-compositor race reliability study (16 fresh-navigation captures, zero variance), and the login chrome's computed transition contract (base + reduced-motion) all at parity |
