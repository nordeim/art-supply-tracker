...
Automated tests (Vitest, 301 tests) pin the studio-domain vocabulary
...
duration-300` slide stays as the live's only studio motion), and
...
| Verification | ✅ Complete | Lint + typecheck + 301 vitest + 25 Playwright E2E + production build clean; VLM screenshot comparison of all four views (plus the mobile views, the sidebar drawer, and the login page at both viewports — including the error states and the reset-confirmation view) vs the live app (parity); 23-check functional smoke suite (CRUD, filters, panels, modals, navigation regression, focus flows) |
